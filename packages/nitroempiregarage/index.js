let impound = mp.colshapes.newSphere(104.89794921875, 6607.32275390625, 31.898794174194336, 5, 0);
mp.events.add("PushE", (player) => {
  if (mp.players.exists(player)) {
    if(impound.isPointWithin(player.position)) {
        if (player.data.faction == "NitroEmpire") {
           player.call("client:nitroempiregarage:openMainMenu");
           //player.data.mainmenu = true;
        } else {
            player.notify("~r~Du arbeitest nicht bei NitroEmpire");
        }        
    }    
  }
});

mp.events.add("server:nitroempiregarage:openMenu",(player) => {
    gm.mysql.handle.query("SELECT v.*, c.firstname, c.lastname FROM vehicles v LEFT JOIN characters c ON c.id = v.charId WHERE v.impounded = '5'",[],function(err,res) {
        if (err) console.log("Error in Select nitroempiregarage Vehicles: "+err);
        if (res.length > 0) {
            var i = 1;
            let vehList = [];
            res.forEach(function(veh) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if(mp.players.exists(player)) player.call("client:nitroempiregarage:openMenu", [JSON.stringify(vehList)]);
                }
                i++;
            });
        } else {
            player.notify("~r~Keine Fahrzeuge in der NitroEmpire Garage");
        }
    });
});

mp.events.add("server:nitroempiregarage:parkout",(player,id) => {
    const one = new mp.Vector3(111.16442108154297, 6606.20166015625, 31.898794174194336);
    const onehead = 71;
    gm.mysql.handle.query("SELECT * FROM vehicles WHERE id = ?",[id],function(err1,res1) {
        ...
        if (getVehicleFromPosition(one, 3).length > 0) {
            player.notify("~r~Stellplatz nicht Frei!");
        } else {
...
mp.events.add("server:nitroempiregarage:parkin", (player,id) => {
        const pos = new mp.Vector3(111.16442108154297, 6606.20166015625, 31.898794174194336);
