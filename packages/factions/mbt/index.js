let pc = mp.colshapes.newSphere(-1151,-1424,5, 1, 0);
let tattoo = mp.colshapes.newSphere(-1154,-1426,5, 2, 0);

mp.events.add("PushE", (player) => {
    if (mp.players.exists(player)) {
        if (player.data.faction == "MBT") {
            if(pc.isPointWithin(player.position) && player.data.mainmenu == false) {
                player.call("client:mbt:createOfficeComputer",[player.data.factionrang]);  
...
mp.events.add("server:mbt:mainMenu", (player,slot,name) => {
    getNearestPlayer(player, 2);   
    if (name == "Mitarbeiterverwaltung")
    {
        player.call("client:mbt:openMemberMenu");
    }
});
mp.events.add("server:mbt:entlassen", (player,itemText) => {
    getNearestPlayer(player, 2);   
    if(currentTarget !== null) {    
        if (currentTarget.data.faction == "MBT") {
            currentTarget.notifyWithPicture("Moonlight Beach Tattoo", "Entlassung", "Deine Anstellung wurde beendet","CHAR_CALL911");
        currentTarget.call("client:mbt:delmarkers");
        player.notifyWithPicture("Moonlight Beach Tattoo", "Mitarbeiterverwaltung","Du hast "+currentTarget.data.firstname + " "+ currentTarget.data.lastname + " entlassen.","CHAR_CALL911");
        ...
        gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?",[currentTarget.data.charId], function(err5,res5) {
            ...
            player.notify("~r~Der Spieler Arbteitet nicht bei Moonlight Beach Tattoo!")
        } 
    }   
});

mp.events.add("server:mbt:befördern", (player,rank) => {
    if (mp.players.exists(player)) {
        getNearestPlayer(player,2);
        if (currentTarget !== null) {
            if (currentTarget.data.faction == "MBT") {
                ...
                player.notify("~r~Die Person ist nicht bei Moonlight Beach Tattoo");
            }
        } else {
...
mp.events.add("server:mbt:mitarbeiter",(player) => {
    gm.mysql.handle.query("SELECT firstname,lastname,factionrang,id FROM characters WHERE faction = 'MBT'",[],function(err,res) {
        ...
        if (res.length > 0) {
            ...
            res.forEach(function(justiz) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if (mp.players.exists(player)) player.call("client:mbt:Memberlist", [JSON.stringify(LSCList)]);
                }
                i++;
            });
        } else {   
            player.notify("Es gibt keine Moonlight Beach Tattoo Mitarbeiter");
        }
     });
});

mp.events.add("server:mbt:mitarbeiterentl",(player,id) => {
...
mp.events.add("server:mbt:einstellen",(player) => {
    getNearestPlayer(player, 2);  
    if (currentTarget !== null) {
        if (currentTarget.data.faction == "Civillian") {
            gm.mysql.handle.query("UPDATE characters SET faction = 'MBT', factionrang = '1',firma = '1' WHERE id = ?",[currentTarget.data.charId], function (err,res) {
                if (err) console.log("Error in Update Faction user: "+err);
                player.notify("~g~Der Bürger wurde eingestellt");
                currentTarget.notify("Sie wurden bei Moonlight Beach Tattoo eingestellt!");
                currentTarget.data.faction = "MBT";
                currentTarget.data.factionDuty = 0;
