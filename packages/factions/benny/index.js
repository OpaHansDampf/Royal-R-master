...
mp.events.add("server:bennys:mainMenu", (player,slot,name) => {
    ...
    {
       if (mp.players.exists(player)) player.call("client:bennys:openMemberMenu");
    }
});
        ...
        if (mp.players.exists(currentTarget)) {
            if(currentTarget !== null) {    
                if (currentTarget.data.faction == "Bennys") {
                    currentTarget.notifyWithPicture("Los Santos Bennys", "Entlassung", "Deine Anstellung wurde beendet","CHAR_CALL911");
                currentTarget.call("client:bennys:delmarkers");
                player.notifyWithPicture("Los Santos Bennys", "Mitarbeiterverwaltung","Du hast "+currentTarget.data.firstname + " "+ currentTarget.data.lastname + " entlassen.","CHAR_CALL911");
                ...
                gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?",[currentTarget.data.charId], function(err5,res5) {
                    ...
                    if (mp.players.exists(player)) player.notify("~r~Der Spieler Arbteitet nicht bei Bennys!")
                } 
            }
        }        
    }      
