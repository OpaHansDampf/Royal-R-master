let tune = mp.colshapes.newSphere(-338.2681884765625, -136.26783752441406, 39.0096321105957, 30, 0);
...
let chief = mp.colshapes.newSphere(-345,-123,39, 1, 0);
...
mp.events.add("server:lsc:mainMenu", (player,slot,name) => {
...
mp.events.add("server:lsc:entlassen", (player,itemText) => {
    ...
    if(currentTarget !== null) {    
        ...
        currentTarget.call("client:lsc:delmarkers");
        ...
        gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?",[currentTarget.data.charId], function(err5,res5) {
            ...
            player.notify("~r~Der Spieler Arbteitet nicht bei LSC!")
        } 
    }   
});
        ...
        if (mp.players.exists(currentTarget)) {
            if (currentTarget !== null) {
                if (currentTarget.data.faction == "LSC") {
                    ...
                    player.notify("~r~Die Person ist nicht bei LSC");
                }
            } else {
        ...
        if (res.length > 0) {
            ...
            res.forEach(function(justiz) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if (mp.players.exists(player)) player.call("client:lsc:Memberlist", [JSON.stringify(LSCList)]);
                }
                i++;
            });
        } else {   
            player.notify("Es gibt keine LSC Beamten");
        }
     });
});

mp.events.add("server:lsc:mitarbeiterentl",(player,id) => {
...
mp.events.add("server:lsc:einstellen",(player) => {
    getNearestPlayer(player, 2);  
    if (currentTarget !== null) {
        if (currentTarget.data.faction == "Civillian") {
            gm.mysql.handle.query("UPDATE characters SET faction = 'LSC', factionrang = '1', firma = '1' WHERE id = ?",[currentTarget.data.charId], function (err,res) {
                if (err) console.log("Error in Update Faction user: "+err);
                player.notify("~g~Der Bürger wurde eingestellt");
                currentTarget.notify("Sie wurden bei LSC eingestellt!");
