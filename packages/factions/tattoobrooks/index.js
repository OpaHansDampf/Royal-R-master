let pc = mp.colshapes.newSphere(1325,-1650,52, 1, 0);
let tattoo = mp.colshapes.newSphere(1322,-1654,52, 2, 0);

mp.events.add("PushE", (player) => {
    if (mp.players.exists(player)) {
        if (player.data.faction == "TattooBrooks") {
...
mp.events.add("server:tb:mainMenu", (player,slot,name) => {
    getNearestPlayer(player, 2);   
    if (name == "Mitarbeiterverwaltung")
    {
        player.call("client:tb:openMemberMenu");
    }
});
mp.events.add("server:tb:entlassen", (player,itemText) => {
    getNearestPlayer(player, 2);   
    if(currentTarget !== null) {    
        if (currentTarget.data.faction == "TattooBrooks") {
            currentTarget.notifyWithPicture("Tattoo Brooks", "Entlassung", "Deine Anstellung wurde beendet","CHAR_CALL911");
        currentTarget.call("client:tb:delmarkers");
        player.notifyWithPicture("Tattoo Brooks", "Mitarbeiterverwaltung","Du hast "+currentTarget.data.firstname + " "+ currentTarget.data.lastname + " entlassen.","CHAR_CALL911");
        ...
        gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?",[currentTarget.data.charId], function(err5,res5) {
            ...
            player.notify("~r~Der Spieler Arbteitet nicht bei Tattoo Brooks!")
        } 
    }   
});

mp.events.add("server:tb:befördern", (player,rank) => {
    if (mp.players.exists(player)) {
        getNearestPlayer(player,2);
        if (currentTarget !== null) {
            if (currentTarget.data.faction == "TattooBrooks") {
                ...
                player.notify("~r~Die Person ist nicht bei Tattoo Brooks");
            }
        } else {
...
mp.events.add("server:tb:mitarbeiter",(player) => {
    gm.mysql.handle.query("SELECT firstname,lastname,factionrang,id FROM characters WHERE faction = 'TattooBrooks'",[],function(err,res) {
        ...
        if (res.length > 0) {
            ...
            res.forEach(function(justiz) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if (mp.players.exists(player)) player.call("client:tb:Memberlist", [JSON.stringify(LSCList)]);
                }
                i++;
            });
        } else {   
            player.notify("Es gibt keine Tattoo Brooks Mitarbeiter");
        }
     });
});

mp.events.add("server:tb:mitarbeiterentl",(player,id) => {
...
mp.events.add("server:tb:einstellen",(player) => {
    getNearestPlayer(player, 2);  
    if (currentTarget !== null) {
        if (currentTarget.data.faction == "Civillian") {
            gm.mysql.handle.query("UPDATE characters SET faction = 'TattooBrooks', factionrang = '1',firma = '1' WHERE id = ?",[currentTarget.data.charId], function (err,res) {
                if (err) console.log("Error in Update Faction user: "+err);
                player.notify("~g~Der Bürger wurde eingestellt");
                currentTarget.notify("Sie wurden bei Tattoo Brooks eingestellt!");
                currentTarget.data.faction = "TattooBrooks";
                currentTarget.data.factionDuty = 0;
