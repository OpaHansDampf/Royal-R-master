...
// Main Menu
mp.events.add("client:nitroempire:openMainMenu", (factionrang) => {
  mainMenujustiz = new Menu("NitroEmpire","",MenuPoint);  
  ...
  mainMenujustiz.ItemSelect.on((item, index) => {
    if (item.Text !== "Rechnung austellen") {
      ...
      mp.events.callRemote("server:nitroempire:mainMenu",nextMenu, item.Text);
...
mp.events.add("client:nitroempire:closeMainMenu", () => {
  mainMenujustiz.Close();
});

//MemberMenu
mp.events.add("client:nitroempire:openMemberMenu", () => {
  memberMenujustiz = new Menu("NitroEmpire","Mitarbeiterverwaltung",MenuPoint);
  ...
  memberMenujustiz.ItemSelect.on((item, index) => {  
     if (item.Text == "Einstellen") {
      mp.events.callRemote("server:nitroempire:einstellen");
      memberMenujustiz.Close();
     } else if (item.Text == "Entlassen") {
       mp.events.callRemote("server:nitroempire:entlassen");
       memberMenujustiz.Close();
     } else if (item.Text == "Befördern") {
      mp.events.callRemote("server:nitroempire:befördern",item.SelectedItem.DisplayText);
      memberMenujustiz.Close();
     }
  });
});
mp.events.add("client:nitroempire:closeMemberMenu", () => {
memberMenujustiz.Close();
});

mp.events.add("client:nitroempire:createChiefMenu",() => {
  let main = new Menu("Chef PC", "Chef Computer", MenuPoint);
    ...
    if (item.Text == "Mitarbeiter") {
      mp.events.callRemote("server:nitroempire:mitarbeiter");
      main.Close();
...
function drawMenu(charJSON){
  ...
  ui_List.ItemSelect.on((item, index) => {      
      mp.events.call("client:nitroempire:memberSub",item.Description);
      ui_List.Close();
  });
}
mp.events.add("client:nitroempire:Memberlist", drawMenu);

mp.events.add("client:nitroempire:memberSub",(id) => {
  ...
  memberSub.Visible = true;
    ...
    if (item.Text == "Kündigen") {
      mp.events.callRemote("server:nitroempire:mitarbeiterentl",id);
      memberSub.Close();
    } else if (item.Text == "Schließen") {
      memberSub.Close();
    }
